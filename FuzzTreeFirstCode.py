...
def _EdgeLabelRespect(x, y, label_edge_ij, len_edges_pattern, nodes_target, label_edge_target, edges_target, IDI_matrix, oriented):
    if x == y: #Si des voisins sont mappés à la même chose on peut les rejeter
        return 0
    ...
    return 0 #0 #currently 0 simply eliminate this pattern 
...
def print_mapping_on_target_graph(GP, GT, mapping = []):
    if mapping == []:
        mapping = main(GP, GT)[0][0]
    GG = GT.copy()
    pos = nx.spring_layout(GG, k=0.2)  # positions for all nodes
...
nx.draw(GP, pos, width=1, linewidths=1)
plt.show()
pos = nx.spring_layout(GT)
nx.draw(GT, pos, width=1, linewidths=1)
plt.show()
mapping = main(GP, GT)
print(mapping)
#sur 10 samples on retrouve : ([(1, ('B', 7)), (2, ('B', 8)), (3, ('B', 12)), (4, ('B', 13)), (5, ('B', 17)), (6, ('B', 18)), (7, ('B', 23)), (8, ('B', 24))], 8)
#print_mapping_on_target_graph(GP, GT, mapping = [(1, ('B', 7)), (2, ('B', 8)), (3, ('B', 12)), (4, ('B', 13)), (5, ('B', 17)), (6, ('B', 18)), (7, ('B', 23)), (8, ('B', 24))])
#En chronométrant à la main le nombre sde samples (sachant qu'ils sont print) ne semblent pas beaucoup impacter la complexité (2 min 20 sec pour 10 samples et 3min12 sec pour 1000  samples)
# En particulier sur 1000 sample fuzzy sur les labels on arrive à retrouver des samples corrects (8 exactement), dont certains qu'on ne trouver pas avant sur la strand E
