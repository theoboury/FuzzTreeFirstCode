...
def first_test_mapping():
   ...
   # with open("1Y27-4falselabelsCHSintoCWHandTHWintoTHSandTHWintoTHSandcWWintoTHS.nxpickle",'rb') as fT:
    ...
    with open("1Y27-1gapinside.nxpickle",'rb') as fT:
        GT = pickle.load(fT)
    #print(GT.nodes.data())
    #print(GT.edges.data())
    timer = time.time()
...
def first_test_varna():
    with open("rin163.pickle",'rb') as fP:
        GP = pickle.load(fP)
    with open("2XNZ.nxpickle", 'rb') as fT:
    #with open("1Y27.nxpickle",'rb') as fT:
        GT = pickle.load(fT)
...
def test_graph_where_pattern_is_detected(GPpath = "rin163.pickle", GTlistfolder = "RNAstorage", BooleanListGPinGT = [0,0,0,0,0], nb_samples=1000, remove_near=True, timeout=800):
    """
